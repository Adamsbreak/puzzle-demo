import { clonePuzzleState } from "./model.js";
import { canPlace, isSolvedState } from "./rules.js";
...
function buildMove(piece, nextRow, nextCol, direction) {
  return {
    pieceId: piece.id,
...
function candidateDirections(piece) {
...
function isRuleActive(rule, state) {
...
function applyEnvironmentRulesOnce(state, rules) {
...
function settleUntilStable(state, rules, maxSteps = 64) {
  let current = cloneSolverState(state);
  const seen = new Set([serializeState(current)]);
  ...
  for (let i = 0; i < maxSteps; i += 1) {
    ...
    const stable = pendingRules.every((rule) => !rule.isStable || rule.isStable(current));
    if (stable) {
      return current;
    }
...
function advanceRuntimeTick(state) {
  const nextState = cloneSolverState(state);
  nextState.runtime.tick += 1;
  return nextState;
}

function determineSolverMode(rules) {
...
function advanceEnvironmentWithRules(state, rules) {
  if (rules.length === 0) {
...
export function createStaticRuleEngine() {
  return {
    mode: "static",
    deterministic: true,
...
export function createEnvironmentRuleEngine(rules) {
...
export function createWindRuleEngine() {
  return createEnvironmentRuleEngine([createWindRule()]);
}

export function resolveEnvironmentRules(state) {
  const rules = [];
...
export function resolveRuleEngine(state) {
  return createEnvironmentRuleEngine(resolveEnvironmentRules(state));
}

export function solvePuzzle(initialState, options = {}) {
  ...
  const seen = new Set([engine.serializeState(start)]);
  let explored = 0;
